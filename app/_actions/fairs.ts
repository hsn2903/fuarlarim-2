...
import { fairSchema, imageSchema, validateWithZodSchema } from "@/lib/schemas";
import { uploadImage } from "@/lib/supabase";
import { FairFilters, TourProgram } from "@/lib/types";
import { renderError } from "@/lib/utils";
import { revalidatePath } from "next/cache";

// Create Fair
export const createFair = async (prevState: unknown, formData: FormData) => {
  try {
    const rawData = Object.fromEntries(formData);
    const validatedFields = fairSchema.parse(rawData);
    const tourPrograms = formData.get("tourPrograms") as string;
    ...
    const validatedLogoImageFile = validateWithZodSchema(imageSchema, {
      image: logoImageFile,
    });
    const validatedCoverImageFile = validateWithZodSchema(imageSchema, {
      image: coverImageFile,
    });

    // Upload Images
    const logoImagePath = await uploadImage(validatedLogoImageFile.image);
    const coverImagePath = await uploadImage(validatedCoverImageFile.image);
    ...
    return { message: "Fuar Oluşturuldu." };
  } catch (error) {
...
export const getGuaranteedDepartureTours = async () => {
  const fairs = await prisma.fair.findMany({
    where: { isDefiniteDepartureTour: true },
  });
  return fairs;
};
